...
public class StudentTest {
    // session的工厂类对象, 是线程安全的, 一个数据库库对应一个session工厂类对象
    // 通常在项目启动时自动初始化, 用Configuration对象创建
    private SessionFactory sessionFactory;

    //真正执行CRUD操作的数据库连接对象, 代表了一次数据库连接, 是非线程安全的
    private Session session;

    //事务对象, 与jdbc中的事务类似, 只不过hibernate的事务不是自动提交的, 而jdbc中的事务会自动提交, hibernate中的事务需要手动提交
    private Transaction transaction;

    /**
     * 初始化操作
     * before注解, 在Test注解执行之前的方法, 通常该方法中做一些初始化的操作
     */
    @Before
    public void init() {
    ...
    /**
     * 销毁操作
     * After注解, 在Test注解执行之后的方法, 通常该方法中做一些释放, 关闭的操作
     */
    @After
    public void destroy() {
    ...
    public void query() {
        ...
        //条件语句中占位符参数对应
        query.setString(0, "张三");// 指定第一个问号对应的值
        // 返回查询结果集
    ...
    public void createCriteria() {
        //获取要查询的Student对应的Criteria接口对象, 它是比HQL更加高级的查询方式
